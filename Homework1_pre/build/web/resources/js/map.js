    ...
    fetch(window.contextPath + '/IncidentController?action=getMapData')
        ...
        .then(function(incidencias) {
            incidencias.forEach(function(incidencia) {
                // Selecciona el icono según el estado de la incidencia.
                // Se espera que la propiedad "state" sea un objeto con "name".
                let iconUrl = 'https://unpkg.com/leaflet@1.9.3/dist/images/marker-icon.png';
                if (incidencia.state && incidencia.state.name) {
                    let stateName = incidencia.state.name.toLowerCase();
                    if (stateName === 'pendiente') {
                        // Ejemplo: incidencia pendiente → icono rojo
                        iconUrl = 'https://example.com/path/to/red-marker.png';
                    } else if (stateName === 'en proceso') {
                        // Ejemplo: incidencia en proceso → icono amarillo
                        iconUrl = 'https://example.com/path/to/yellow-marker.png';
                    } else if (stateName === 'cerrado') {
                        // Ejemplo: incidencia cerrada → icono verde
                        iconUrl = 'https://example.com/path/to/green-marker.png';
                    }
                }
                ...
                // Se usan las propiedades "latitude" y "longitude" obtenidas del JSON.
...
function loadIncidents() {
    fetch('http://localhost:8080/Homework1_pre/incidencia')  // Cambia esta URL por la correcta
        .then(response => response.json())
        .then(data => {
            // Aquí verificamos que las incidencias contienen las coordenadas x (latitud) y y (longitud)
