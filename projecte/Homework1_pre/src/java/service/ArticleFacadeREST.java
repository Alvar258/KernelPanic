...
public class ArticleFacadeREST {
    ...
    public Response updateArticle(@PathParam("id") Long id, Article updatedArticle, @Context SecurityContext securityContext) {
        ...
        if (updatedArticle.getTopics() != null) {
            // Obtener los topics de la base de datos
            List<String> topicNames = updatedArticle.getTopics().stream()
                .map(topic -> topic.getName())
                .collect(Collectors.toList());

            List<Topic> topics = em.createQuery("SELECT t FROM Topic t WHERE t.name IN :names", Topic.class)
                .setParameter("names", topicNames)
                .getResultList();

            // Limpiar los topics actuales
            article.getTopics().clear();
            
            // Añadir los nuevos topics
